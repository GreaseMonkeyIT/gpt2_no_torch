...
I worked through Karpathy's zero-to-hero and derived every gradient by hand; under
the compressed deadline I had Claude build the implementation from those derivations
and record the process as we went. What follows is my journal: roadblocks and
reflections, with the human/AI split flagged where it happened, and stated in full in the 
README's [AI use disclosure](README.md#ai-use-disclosure).
...
- Env: pyenv-win Python 3.12.10, venv, numpy-only training path.
  ...
  ~9B tokens — far beyond weekend compute on an RTX 3050).
...
- Derivations done on paper: most of this was covered at college, in 1st and 2nd semesters,
  and again in my AI-in-Digital-Signal-Processing minor.
- Engine progress: mostly indicated by the design choices mentioned in README.md
...
- Also mine: the makemore MLP ([notebooks/warmups/02_mlp_makemore.ipynb](notebooks/warmups/02_mlp_makemore.ipynb)) —
  the fixed-window char model, train 2.126 / dev 2.170 (matching the video); its
  concat-slot position handling is the contrast the transformer's learned wpe replaces.
...
Character level model has to be honest to the Shakespeare-ness: glimpses of the rhymes, and the iambic pentameter are visible, along with the models attempt at using Victorean era wording. "obstrocks" and "consulk'd".
...
BPE tokens mean no misspellings ever; coherence lasts for approx one clause. Also: the
...
content starts orbiting around itself: "a reality of life is a reality of life" symptoms of the small-model. Grammar is learned before meaning is achieved by the model.
...
New at this stage is the correct quote, and correct dialogue mechanics exhibited
(open/close quotes, questions inside quotes, attribution), and named entities as 
reusable referents. But coreference degenerates, the father, sister, and brother are all
named "Paul". The repetition climbed a level of abstraction: at 82M tokens
it looped phrases, at 194M it is looping entities. Coherence horizon has risen to almost 2 
sentences, then topic drift (philosophy -> head coach -> family tree).
...
The naming collapse is gone, and now a pronoun referent ("her") now survives across a
...
the entire sample. Each stage of training didn't remove the repetition; it
...
computed over the full 1.12M-token val set on the last checkpoint, not from the
20-block estimator.
...
- Losses: train 10.86 → 4.181; final 20-block val **4.1666 (ppl 64.5)**,
  ...
  the headline number still needs the full 1.12M-token val pass.
...
- Final evals (done): full 1.12M-token val pass on the final
  checkpoint — nll 4.1433, **ppl 63.01**, the headline number the 20-block
  estimator only approximated (63.0 vs the noisy 64.5). Downstream, forward-only:
  WikiText-2 **ppl 203.9** (nll 5.3177 over 286K tokens) and LAMBADA **7.8%**
  (39/500, greedy exact-match), both far from GPT-2 124M's 29.4 / 46.0% at 1/4
  the params and 1/40 the tokens, LAMBADA still well above chance. Loss +
  perplexity curves in `plots/`; final samples and the results table are in the
  README.
